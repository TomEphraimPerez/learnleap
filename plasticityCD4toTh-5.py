...
for ind, value in enumerate(Pts):
    ...
    attribute[0] = None
    while flag:
        attribute[0] = input("\tplease input a valid float for attribute for: " + a)
        match_val = re.match("[-+]?\\d+([/.]\\d+)?$", attribute[0])
        if match_val is None:
            print("\n\t\tPlease enter a vAliD decimal number.")
        else:
            flag = False
    number = float(attribute[0])
    print("\tThe input attr number is:", number, 'for' + attribute[0] )
