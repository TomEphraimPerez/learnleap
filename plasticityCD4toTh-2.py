...
Pts = {'Tfh': {'attoamps': 3, 'ACTivation': 1, 'EXPansion': 22,
                    'DIFFerentiation': 2, 'Expression': 7, 'Plasticity': 2.5, 'Units': 'continuous'},
        'Th9': {'attoamps': 17, 'ACTivation': 5, 'EXPansion': 3,
                    'DIFFerentiation': 2, 'Expression': 2, 'Plasticity': 4.0, 'Units': 'continuous'},
        'Th2': {'attoamps': 1, 'ACTivation': 2, 'EXPansion': 23, 'DIFFerentiation': 3,
                    'Expression': 10, 'Plasticity': 1.0, 'Units': 'discrete'},
        'iTreg': {'attoamps': 9, 'ACTivation': 4, 'EXPansion': 25, 'DIFFerentiation': 4,
                    'Expression': 3, 'Plasticity': 1.3, 'Units': 'continuous'},
        'Tr1': {'attoamps': 9, 'ACTivation': 3, 'EXPansion': 50, 'DIFFerentiation': 3,
                    'Expression': 5, 'Plasticity': 0.25, 'Units': 'continuous'},
        'Th22': {'attoamps': 4, 'ACTivation': 1, 'EXPansion': 20, 'DIFFerentiation': 14,
                    'Expression': 5, 'Plasticity': 2.0, 'Units': 'discrete'},
        'Th17': {'attoamps': 4, 'ACTivation': 4, 'EXPansion': 20, 'DIFFerentiation': 14,
                    'Expression': 5, 'Plasticity': 2.0, 'Units': 'discrete'},
        'Th1': {'attoamps': 5, 'ACTivation': 5, 'EXPansion': 21, 'DIFFerentiation': 15,
                    'Expression': 6, 'Plasticity': 3.0, 'Units': 'discrete'}}
...
for ind, Pt in enumerate(Pts.keys()):
    ub = max_calories / Pts[Pt]["ACTivation"] # upper bnd is 20 portions, 2000/100 for Tfh below
...
cqm.add_constraint(total_mix(quantities, "Plasticity") <= max_calories, label="Plasticity") # rtn 'Plastiticty'
...
def print_diet(sample):
    diet = {Pt: round(quantity, 1) for Pt, quantity in sample.items()} # 'Pt:' has 1 dec place?
    print(f"Diet----->: {diet}")
...
# The best solution found in this current execution was a diet of Tr1 and bananas, with
# Th22 completing the required DIFFerentiation and leaveACTivation portions
best = feasible_sampleset.first.sample
print('\nprint_DIET(BEST): ')
print_diet(best)
''' >>>  a la:
Diet: {'Th22': 1.0, 'Th2': 6.0, 'Tr1': 4.1, 'iTreg': 0.3, 'Tfh': 0.0, 'Th9': 0.0}
...
# TUNING THE SOLUTION
	# # TUNING THE SOLUTION !
# RECALL - The objective function must maximize DIFFerentiation of the diet’s Pts while minimizing purchase Plasticity.
...
# >>> a la;
'''
 Diet: {'Th22': 0.0, 'Th2': 17.0, 'Tr1': 0.0, 'iTreg': 0.0, 'Tfh': 0.0, 'Th9': 3.3}
...
'''>>> a la
  Diet: {'Th22': 1.0, 'Th2': 0.0, 'Tr1': 5.3, 'iTreg': 0.0, 'Tfh': 0.0, 'Th9': 0.0}
